...
class Alus_hmvc extends CI_Model
{
   ...
   function get_menu_html($menu_list, $root_menu_id = 0)
   {
      ...
      while ($loop && (($option = each($children[$parent])) || ($parent > $root_menu_id))) {
         if ($option === false) {
            ...
            $is_active = $this->is_child_active($children, $option['value']['menu_id']);
            $active_class = $is_active ? ' active menu-open' : '';
            ...
            if (count($parent_stack) == 1) {
               ...
               $li_class = 'treeview' . ($is_active ? ' active menu-open' : '');

               $this->html[] = sprintf(
                  '%1$s<li class="treeview%6$s">
      <a href="#">
         <i class="%5$s" aria-hidden="true"></i>
         <span> %3$s </span>
         <span class="pull-right-container">
            <i class="fa fa-angle-left pull-right"></i>
                  ...
                  $tab,
                  $option['value']['menu_uri'],
                  $option['value']['menu_nama'],
                  $option['value']['menu_target'],
                  $option['value']['menu_icon'],
                  $active_class
               );
            }
   ...
   private function is_child_active($children, $parent_id)
   {
      $current_uri = $this->uri->segment(1);
      ...
      foreach ($children[$parent_id] as $child) {
         if ($child['menu_uri'] == $current_uri) {
            return true;
         }

         // cek level lebih dalam (recursive)
         if ($this->is_child_active($children, $child['menu_id'])) {
            return true;
         }
      }
